...
channel_sales = df.groupby(df['channel'])['total_revenue'].mean().sort_values(ascending=False).reset_index()
top3_channel_sales = channel_sales[channel_sales['channel'] != 'Nan'].head(3)
print(f"топ-3 рекламных канала по среднему чеку \n{top3_channel_sales}")
region_sales = df.groupby(df['region'])['total_revenue'].mean().sort_values(ascending=False).reset_index()
top3_region_sales = region_sales[region_sales['region'] != 'Nan'].head(3)
print(f"топ-3 региона по среднему чеку\n{top3_region_sales}")
region_month_sales = df.groupby(['month', 'region'])['total_revenue'].mean().sort_values(ascending=False).reset_index()
top3_region_month_sales = region_month_sales[region_month_sales['region'] != 'Nan'].head(4)
print(f"топ-3 месяца по среднему чеку с разбивкой по региону \n{top3_region_month_sales}")
mau_month_channel = (df.groupby(['channel', 'month'])['user_id'].nunique().sort_values(ascending=False)
                     .reset_index())
print(f"mau по каждому месяцу с разбивкой по регионам \n{mau_month_channel}")
mau_month_channel = mau_month_channel.groupby(['channel'])['user_id'].mean().sort_values(ascending=False).reset_index()
top3_mau_month_channel = mau_month_channel[mau_month_channel['channel'] != 'Nan'].head(3)
...
ax7.set_yticks(x2, ['Nan', 'email-рассылки', 'organic', 'контекстная реклама', 'реклама у блогеров', 'социальные сети'])
